...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	if(empty($input_employee_irs)){
		...
		if($result->num_rows == 1){
			...
			$employee_irs_err = 'Please enter a valid customer irs.';
		}
	}
    ...
    if(empty($start_date_err) && empty($finish_date_err) && empty($customer_irs_err) && empty($room_id_err) && empty($employee_irs_err)){
		...
		$sql = "UPDATE Rents SET RStart_Date = ?, RFinish_Date = ? ,Employee_IRS_Number_E = ?, Customer_IRS_Number_C = ?, Hotel_room_Room_ID = ? 
				WHERE Rents_id = ?";

		
         if($stmt = $conn->prepare($sql)){
            // Bind variables to the prepared statement as parameters
             $stmt->bind_param("ssiiii", $param_start_date, $param_finish_date, $param_employee_irs, 
				$param_customer_irs, $param_room_id, $param_rent_id);
    ...
    if(isset($_GET["rent_id"]) && !empty(trim($_GET["rent_id"]))){
        // Get URL parameter
        $rent_id =  trim($_GET["rent_id"]);
        
        // Prepare a select statement
        $sql = "SELECT * FROM Rents WHERE Rents_id = ?";
      if($stmt = $conn->prepare($sql)){
            // Bind variables to the prepared statement as parameters
            ...
            if($stmt->execute()){
                ...
                if($result->num_rows == 1){
                    ...
                    $start_date = $row["RStart_Date"];
					$finish_date = $row["RFinish_Date"];
					$employee_irs = $row["Employee_IRS_Number_E"];
    ...
    <div class="wrapper">
        <div class="container-fluid">
			...
			<div class="form-group <?php echo (!empty($finish_date_err)) ? 'has-error' : ''; ?>">
						...
						<input type="hidden" name="Rents_id" value="<?php echo $rent_id; ?>"/>
