...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($start_date_err) && empty($finish_date_err) && empty($customer_irs_err) && empty($room_id_err) ){
		...
		$sql = "INSERT INTO Reserves (	ResStart_Date, ResFinish_Date, 	ResPaid, 	Customer_IRS_Number_C, 	Hotel_room_Room_ID) 
					VALUES (?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE Customer_IRS_Number_C = Customer_IRS_Number_C";

		
         if($stmt = $conn->prepare($sql)){
            // Bind variables to the prepared statement as parameters
             $stmt->bind_param("sssii", $param_start_date, $param_finish_date, $param_paid, $param_customer_irs, $param_room_id);
