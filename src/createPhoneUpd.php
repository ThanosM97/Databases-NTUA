...
$next=1;
	...
	while(isset($_POST["phone".$next])){
		$sql = "INSERT INTO Hotel_Phone_Number (HPhone_Number, Hotel_Hotel_ID , Hotel_Hotel_Group_Hotel_Group_ID) VALUES (?,?,?)";
		$stmt = $conn->prepare($sql);
		$param_phone = trim($_POST["phone".$next]);
		// Bind variables to the prepared statement as parameters
		$stmt->bind_param("iii",$param_phone, $hotel_id , $group_id );
		$next = $next +1;
		if (!$stmt->execute()){
...
header("location: updateHotel.php?Group_ID=$group_id&Hotel_ID=$hotel_id");
exit(); 
  ...
  <script>
    ...
    <div class="wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header">
                        <h2>Add phones to hotel: hotel ID= <?php echo $_GET["hotelid"];?> and Group ID =<?php echo $_GET["groupid"];?> .</h2>
                    </div>
                    <p>Please fill this form to add more phones to the hotel with id: <?php echo $_GET["hotelid"]; ?>.</p>
			...
			<input type="hidden" name="hotel_id" value=<?php echo $_GET["hotelid"]; ?> >
